...
import { FontTypes, IconNames, InputSizes, InterestPostParams, PostType } from "@/types/Components"
    ...
    return (
        <FlatList
            ...
            renderItem={({ item, index }) => {
                if (tabs?.tab !== tabNames[2]) {
                    return <CommunityPostCard key={index} data={item} uid={uid} cols={2} scheduled={tabs?.visibility === POST_VISIBILITY.SCHEDULED} navigationPath="/community-post" />
                } else {
